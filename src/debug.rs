    ...
    loop {
        ...
        match std::io::stdin().read_line(&mut input) {
            Ok(_) => if input.starts_with("b:") {
                ...
                // Print state
                println!("{}", cpu.get_state().to_string());
...
fn print(s: &str, cpu: &mut CPU) {
    match s {
        "a" => println!("a: ${:04X}", cpu.get_state().a),
        "x" => println!("x: ${:04X}", cpu.get_state().x),
        "y" => println!("y: ${:04X}", cpu.get_state().y),
        "s" => println!("s: ${:04X}", cpu.get_state().s),
        "db" => println!("db: ${:02X}", cpu.get_state().db),
        "dp" => println!("dp: ${:04X}", cpu.get_state().dp),
        "pb" => println!("pb: ${:02X}", cpu.get_state().pb),
        "pc" => println!("pc: ${:04X}", cpu.get_state().pc),
        "p" => println!("p: b{:08b}", cpu.get_state().p),
        "e" => println!("e: b{:08b}", cpu.get_state().pe),
        s => {
            // Memory range
            if let Some(x) = s.find('-') {
                ...
                match u32::from_str_radix(s, 16) {
                    Ok(num) => println!("${:06X}: ${:02X}", num, cpu.get_mem_at(num)),
                    Err(e) => println!("Invalid p tag: {}", e),
                }
            }
        }
...
fn step_and_trace(cpu: &mut CPU, stack_trace: &mut Vec<u32>, print: bool) {
    let instr = cpu.get_instr();
    match instr[0] {
        0x22 | 0x20 | 0xFC => {
            stack_trace.push(make24!(cpu.get_state().pb, cpu.get_state().pc));
        },
        0x6B | 0x60 => {
            stack_trace.pop();
        },
